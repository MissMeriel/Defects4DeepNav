...
class CombinedDataSequence(data.Dataset):
    def __init__(self, roots, image_size=(100,100), transform=None, robustification=False, noise_level=10):
        """
        Args:
            root_dir (string): Directory with all the images.
            transform (callable, optional): Optional transform to be applied
        ...
        for r in self.roots:
            for p in Path(r).iterdir():
                if p.is_dir() and marker in str(p):
                    print("{}/{}".format(p.parent,p.stem.replace(marker, "")))
                    self.dirs.append("{}/{}".format(p.parent,p.stem.replace(marker, "")))
                    image_paths = []
                    try:
                        self.dfs_hashmap[f"{p}"] = pd.read_csv(f"{p}/data_cleaned.csv")
    ...
    def __getitem__(self, idx):
        ...
        if len(df_index) != 1:
            # Print all items in df_index
            print(f"All items in Error df_index: {df_index.tolist()}")
            raise ValueError(f"Expected exactly one row for {img_name.name}, found {len(df_index)} rows (df_index) in directory. df is {df}. pathobj is {pathobj}") 
            #: {self.root}. df is {df}. pathobj is {pathobj}")
