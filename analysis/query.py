...
This functionality provides total samples, size on disk, and duration of the failure in seconds 
as well as a count of failures per model and summed trace attributes for all failures per model. 
...
def parse_arguments():
    parser = argparse.ArgumentParser(prog='query',
                    ...
                    epilog='Generate attributes referred to in the paper, Table II column 3')
...
def main(args):
    ...
    print(f"Total failure traces: {len(tracedirs)}")
    for td in tracedirs:
